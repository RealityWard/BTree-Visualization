...
{
  internal class GUINode : IComparable<GUINode>
  {
    private long _ID;
...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
    ...
    public int CompareTo(GUINode? other)
    {
      ...
      int rhsMax = int.MinValue;
      for (int i = 0; i < other.NumKeys; i++)
      {
        if (other.Keys[i] > rhsMax)
        {
          rhsMax = other.Keys[i];
        }
      }
    ...
    public static bool operator <=(GUINode lhs, GUINode rhs)
    {
      ...
      {
        int lhsMax = int.MinValue;
        for (int i = 0; i < lhs.NumKeys; i++)
        {
          if (lhs.Keys[i] > lhsMax)
          {
            lhsMax = lhs.Keys[i];
          }
        }
        int rhsMin = int.MaxValue;
        for (int i = 0; i < rhs.NumKeys; i++)
        {
          if (rhs.Keys[i] < rhsMin)
          {
            rhsMin = rhs.Keys[i];
          }
        }
        return lhsMax <= rhsMin;
      }
    }
    ...
    public static bool operator >=(GUINode lhs, GUINode rhs)
    {
      ...
      {
        int lhsMin = int.MaxValue;
        for (int i = 0; i < lhs.NumKeys; i++)
        {
          if (lhs.Keys[i] < lhsMin)
          {
            lhsMin = lhs.Keys[i];
          }
        }
        ...
        for (int i = 0; i < rhs.NumKeys; i++)
        {
          if (rhs.Keys[i] > rhsMax)
          {
            rhsMax = rhs.Keys[i];
          }
        }
        return lhsMin >= rhsMax;
      }
    }
